...
from mvp_ui import _five_period_records, _pl_calculated, _pl_edit_frame, _segment_matrix
...
def test_segment_matrix_has_company_and_own_forecast_columns_even_when_blank():
    ...
    assert "会社予想" in matrix.columns
    assert "独自予想" in matrix.columns
    assert matrix.iloc[0]["2026年3月期 実績"] == pytest.approx(100)


def test_pl_editor_uses_empty_text_instead_of_none():
    frame = _pl_edit_frame([{"fiscal_year": "2026年3月期", "result_type": "実績", "sales": 1000}])
    assert frame.loc[2, "売上高"] == "1,000"
    assert frame.loc[2, "EPS"] == ""
    assert "None" not in frame.astype(str).to_string()
